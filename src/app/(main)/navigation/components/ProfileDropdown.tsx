    ...
    >
      <>        {/* User Identity Section - Avatar Only */}
        <div className="pt-5 pb-3 flex justify-center items-center border-b">
          ...
          {(user.userLevel?.position || user.profile?.jobTitle) && (
            <div className="px-4 py-2 space-y-1 border-b">
              {user.userLevel?.position && (
                <div className="flex items-center text-sm text-gray-700">
                  ...
                  <span className="truncate">{user.userLevel.position}</span>
                </div>
              )}
          ...
          {(user.profile?.region || user.profile?.province || user.profile?.city || user.profile?.barangay) && (
            <div className="px-4 py-2 space-y-1 border-b">
              <div className="flex items-start text-sm text-gray-700">
                ...
                <div>
                  ...
                  {user.profile?.region && <div className="ml-2 text-xs truncate"><span className="text-gray-500">Region:</span> {user.profile.region}</div>}
                  {user.profile?.province && <div className="ml-2 text-xs truncate"><span className="text-gray-500">Province:</span> {user.profile.province}</div>}
                  {user.profile?.city && <div className="ml-2 text-xs truncate"><span className="text-gray-500">City:</span> {user.profile.city}</div>}
                  {user.profile?.barangay && <div className="ml-2 text-xs truncate"><span className="text-gray-500">Barangay:</span> {user.profile.barangay}</div>}
                </div>
              </div>
            </div>
          )}
